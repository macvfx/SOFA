...
# headers (added firewall_enabled, filevault_status, filevault_recovery_key, sip_enabled)
echo "\"name\",\"device_name\",\"serial\",\"os_version\",\"latest_major_os\",\"needs_update\",\"product_name\",\"filevault_status\",\"filevault_recovery_key\",\"sip_enabled\",\"firewall_enabled\",\"latest_compatible_os\",\"latest_compatible_os_version\",\"last_seen_at\"" > "$FULL_CSV"
echo "\"name\",\"device_name\",\"serial\",\"os_version\",\"latest_major_os\",\"product_name\",\"filevault_status\",\"last_seen_at\"" > "$NEEDS_UPDATE_CSV"
...
echo "$response" | jq -c '.data[]' | while read -r device; do
    ...
    # FileVault, SIP, and Firewall fields
    ...
    # Write full device info (with FileVault and Firewall fields)
    echo "\"$name\",\"$device_name\",\"$serial\",\"$os_version\",\"$latest_major_os\",\"$needs_update\",\"$product_name\",\"$filevault_status\",\"$filevault_recovery_key\",\"$sip_enabled\",\"$firewall_enabled\",\"$latest_compatible_os\",\"$latest_compatible_os_version\",\"$last_seen_at_fmt\"" >> "$FULL_CSV"

    if [[ "$needs_update" == "yes" ]]; then
        echo "\"$name\",\"$device_name\",\"$serial\",\"$os_version\",\"$latest_major_os\",\"$product_name\",\"$filevault_status\",\"$last_seen_at_fmt\"" >> "$NEEDS_UPDATE_CSV"
    fi
done
...
echo "  → Raw SimpleMDM API response JSON: ${OUTPUT_DIR}/simplemdm_raw_response_${DATE}.json"
open "$FULL_CSV"
open "$NEEDS_UPDATE_CSV"
open "$SUPPORTED_CSV"
open "$OUTPUT_DIR"/simplemdm_raw_response_${DATE}.json
