...
# headers (no xprotect column)
echo "\"name\",\"device_name\",\"serial\",\"os_version\",\"latest_major_os\",\"needs_update\",\"product_name\",\"latest_compatible_os\",\"latest_compatible_os_version\",\"last_seen_at\"" > "$FULL_CSV"
echo "\"name\",\"device_name\",\"serial\",\"os_version\",\"latest_major_os\",\"product_name\",\"last_seen_at\"" > "$NEEDS_UPDATE_CSV"
...
echo "$response" | jq -c '.data[]' | while read -r device; do
    ...
    echo "\"$name\",\"$device_name\",\"$serial\",\"$os_version\",\"$latest_major_os\",\"$needs_update\",\"$product_name\",\"$latest_compatible_os\",\"$latest_compatible_os_version\",\"$last_seen_at_fmt\"" >> "$FULL_CSV"

    if [[ "$needs_update" == "yes" ]]; then
        echo "\"$name\",\"$device_name\",\"$serial\",\"$os_version\",\"$latest_major_os\",\"$product_name\",\"$last_seen_at_fmt\"" >> "$NEEDS_UPDATE_CSV"
    fi
done
